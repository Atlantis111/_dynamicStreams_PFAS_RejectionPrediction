...
if __name__ == "__main__":
    ...
    try:
        ...
        if latest_version >= 2:
            ...
            # =====================================================================
            # [核心新增]：利用 version_data 绘制核心特征的 PDP / SHAP 依赖演变图
            # 请确保将这段代码放在 try 块的最末尾（heatmap 绘制代码之后）
            # =====================================================================
            def plot_pdp_evolution(model_instance, feature_name):
                import matplotlib
                ...
                print(f"\n正在绘制特征 {feature_name} 的 PDP 演变图...")

                # 我们提取 v1 到 v4 进行绘制
                versions_to_plot = [1, 2, 3, 4]
                available_versions = [v for v in versions_to_plot if v in model_instance.version_data]
                ...
                for i, version in enumerate(available_versions):
                    if i >= 4: break  # 只画 2x2 四张子图

                    # 1. 加载对应版本的历史模型
                    model_path = os.path.join(snapshot_dir, f"xgb_model_v{version}.model")
                    if not os.path.exists(model_path):
                        continue
                    history_model = xgb.Booster()
                    history_model.load_model(model_path)

                    # 2. 提取该模型训练时的对应截面数据
                    df_x_original = model_instance.version_data[version]['X']

                    # SHAP explainer 需要标准化的输入，但画图的 X 轴我们需要原始的物理数值
                    X_scaled = model_instance.scaler.transform(df_x_original)
                    explainer = shap.TreeExplainer(history_model)
                    shap_values = explainer.shap_values(X_scaled)
                    ...
                    shap.dependence_plot(
                        feature_name,
                        shap_values,
                        df_x_original,  # 使用未标准化的原始数据，使得横坐标具有实际物理意义
                        display_features=df_x_original,
                        ax=axes[i],
                        show=False
                    )

                    axes[i].set_title(f'Model v{version}', fontsize=14, fontweight='bold')
                    axes[i].text(-0.1, 1.05, panel_labels[i], transform=axes[i].transAxes,
                                 fontsize=16, fontweight='bold', va='bottom', ha='right')

                plt.tight_layout(pad=3.0)
                safe_feat_name = feature_name.replace("/", "_").replace(" ", "_").replace("(", "").replace(")", "")
                save_path = f'PDP_evolution_{safe_feat_name}.png'
                plt.savefig(save_path, dpi=600, bbox_inches='tight')
                print(f"✅ {feature_name} 的 PDP 演变图已保存至: {save_path}")
                plt.show()


            # 执行绘图函数：分别绘制空间位阻和热力学指标
            plot_pdp_evolution(psi_model, 'rs/rp')
            plot_pdp_evolution(psi_model, '∆Gs-m (J·m-2)')
