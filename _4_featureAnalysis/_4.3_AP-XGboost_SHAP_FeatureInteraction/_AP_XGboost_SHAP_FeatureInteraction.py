...
class DynamicXGBoost:
    ...
    def shap_feature_analysis(self, X_data=None, y_data=None, top_features=15, figsize=(14, 11)):
        """
        执行SHAP特征重要性分析并生成学术级 2x2 特征交互图组合 (图 a, b, c, d)
        """
        import string
        ...
        # 数据标准化 (仅用于计算SHAP值，画图时我们用真实值)
        X_scaled = self.scaler.transform(X)

        print("开始绘制指定特征的 2x2 SHAP 交互图...")
        ...
        feature_pairs = [
            ('rs/rp', 'pH'),
            ('rs/rp', 'MB contact angle (°)'),
            ('∆Gs-m (J·m-2)', 'Compound log K ow'),
            ('pH', 'MB volume charge density δm (mol·m-3)')
        ]
        ...
        for i, (feat1, feat2) in enumerate(feature_pairs):
            ...
            if feat1 not in X.columns or feat2 not in X.columns:
                print(f"⚠️ 警告: 特征 '{feat1}' 或 '{feat2}' 不存在，跳过子图 ({alphabet[i]})")
                continue
            ...
            # 提取数据：X轴为特征1的真实值，Y轴为特征1的SHAP值，颜色映射为特征2的真实值
            x_vals = X.iloc[:, idx1].values
            y_shap = shap_array[:, idx1]
            c_vals = X.iloc[:, idx2].values

            # 添加 0 的水平基准线，方便快速分辨正负贡献
            ax.axhline(0, color='#666666', linestyle='--', linewidth=1.2, alpha=0.5, zorder=0)

            # 绘制散点图，使用学术常用的 coolwarm 冷暖色盘
            sc = ax.scatter(
                x_vals, y_shap,
                c=c_vals, cmap='coolwarm',
                s=28, alpha=0.85, edgecolors='none', zorder=2
            )

            # ================= 坐标轴与文字排版 =================
            ax.set_title(f'({alphabet[i]}) Interaction: {feat1} & {feat2}',
                         loc='left', fontsize=14, fontweight='bold', pad=12)

            ax.set_xlabel(feat1, fontsize=12, fontweight='bold')
            ax.set_ylabel(f'SHAP Value ({feat1})', fontsize=12, fontweight='bold')
            ...
            # ================= 添加子图专属 Colorbar =================
            cbar = plt.colorbar(sc, ax=ax, pad=0.03, aspect=25)
            # 缩短颜色条的特征名，防止挤占画幅（可选）
            cbar_label = feat2 if len(feat2) < 25 else feat2[:22] + "..."
        ...
        interaction_path_png = f"shap_plots/shap_interaction_2x2_{timestamp}.png"
        interaction_path_pdf = f"shap_plots/shap_interaction_2x2_{timestamp}.pdf"
        plt.savefig(interaction_path_png, dpi=400, bbox_inches='tight', format='png')
        plt.savefig(interaction_path_pdf, dpi=400, bbox_inches='tight', format='pdf')
        print(f"\n学术级 2x2 特征交互图已保存: \n-> {interaction_path_png}\n-> {interaction_path_pdf}")
        plt.close()

        return {
            'interaction_plots': interaction_path_png,
