...
class DynamicXGBoost:
    ...
    def shap_feature_analysis(self, X_data=None, y_data=None, top_features=12, figsize=(15, 4.5)):
        """
        执行特征依赖分析并生成纯净版学术级 PDP 曲线图 (图 a, b, c)
        无散点，聚焦特定特征。兼容原有的调用参数。
        """
        import string
        ...
        # 严格指定你需要的三个特征
        target_features = ['rs/rp', 'pH', 'MB contact angle (°)']

        # ================= 学术级绘图全局设置 =================
        plt.rcParams['font.family'] = 'Times New Roman'
        plt.rcParams['axes.linewidth'] = 1.2

        # 刚好3个特征，采用 1行3列 的经典排版，使用传入的 figsize 或者默认的 (15, 4.5)
        ...
        for i, feature in enumerate(target_features):
            ...
            if feature not in X.columns:
                print(f"⚠️ 警告: 特征 '{feature}' 不在数据集中，跳过。")
                continue
            ...
            # 计算分箱均值作为 PDP 趋势曲线的基础
            bins = np.linspace(x_vals.min(), x_vals.max(), 20)
            ...
            if len(bin_centers) > 3:
                try:
                    # 使用平滑插值让曲线更优美
                    spl = make_interp_spline(bin_centers, bin_means, k=2)
                    x_smooth = np.linspace(bin_centers.min(), bin_centers.max(), 200)
                    y_smooth = spl(x_smooth)
                    # 使用经典学术沉稳蓝
                    ax.plot(x_smooth, y_smooth, color='#2878B5', linewidth=2.5, zorder=2)
                except Exception:
                    # 如果插值失败退化为直线连接
                    ax.plot(bin_centers, bin_means, color='#2878B5', linewidth=2.5, zorder=2)
            else:
                ax.plot(bin_centers, bin_means, color='#2878B5', linewidth=2.5, zorder=2)
            ...
            # 设置 (a) (b) (c) 标题
            ax.set_title(f'({alphabet[i]}) {feature}', loc='left', fontsize=15, fontweight='bold', pad=12)

            # 横轴名称直接用特征名即可，不需要冗余的“Feature Value”
            ax.set_xlabel(feature, fontsize=13, fontweight='bold')
            ax.set_ylabel('SHAP Value (Impact on Output)', fontsize=13, fontweight='bold')
        ...
        # 保存为高分辨率 PNG 和 PDF
        combined_pdp_path_png = f"shap_plots/shap_pdp_3features_{timestamp}.png"
        combined_pdp_path_pdf = f"shap_plots/shap_pdp_3features_{timestamp}.pdf"
        plt.savefig(combined_pdp_path_png, dpi=400, bbox_inches='tight', format='png')
        plt.savefig(combined_pdp_path_pdf, dpi=400, bbox_inches='tight', format='pdf')

        print(f"\n学术级 3子图 PDP (纯曲线版) 已保存: \n-> {combined_pdp_path_png}\n-> {combined_pdp_path_pdf}")
        plt.close()

        return {
            'combined_pdp_path': combined_pdp_path_png,
